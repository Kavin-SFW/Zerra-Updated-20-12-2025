...
const DataSources = () => {
  ...
  const handleFileUpload = async (files: File[]) => {
    ...
    // Check for duplicate files and schema changes
    const existingSource = dataSources.find((s) =>
      s.name === file.name && s.status !== "error"
    );
    if (existingSource) {
      try {
        const newFileHeaders = await getFileHeaders(file);
        const existingHeaders = existingSource.schema_info?.columns || [];

        const hasSchemaChange =
          newFileHeaders.length !== existingHeaders.length ||
          !newFileHeaders.every((h, i) => h === existingHeaders[i]);

        if (hasSchemaChange) {
          toast.error(
            `File "${file.name}" already exists with a different schema. Please delete the existing file before uploading the new one.`,
            { duration: 6000 },
          );
          return;
        } else {
          toast.error(
            `File "${file.name}" already exists. Please delete the existing file before uploading.`,
            { duration: 5000 },
          );
          return;
        }
        ...
        // If we can't validate inputs, default to blocking duplicate names
        toast.error(
          `File "${file.name}" already exists. Please delete the existing file first.`,
          { duration: 5000 },
        );
        return;
      }
